...
#[pyfunction]
#[pyo3(signature = (table, filters, fields=None))]
fn find_one(
    py: Python<'_>,
    table: String,
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        let res = engine::db::generic_query(&pool, &url, &table, filters, Some(1), fields)
            .await
            .map_err(bridge_error_to_py)?;

        if res.is_empty() {
            Ok(None::<HashMap<String, String>>)
        } else {
            Ok(Some(res[0].clone()))
        }
    })
...
#[pyfunction]
#[pyo3(signature = (table, filters, limit=None, fields=None))]
fn fetch_all(
    py: Python<'_>,
    table: String,
    filters: HashMap<String, String>,
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        let res = engine::db::generic_query(&pool, &url, &table, filters, limit, fields)
            .await
            .map_err(bridge_error_to_py)?;
        Ok(res)
    })
...
fn fetch_lazy(
    ...
    let stream = engine::db::query_lazy(pool, url, &table, filters, limit, fields)
        .map_err(bridge_error_to_py)?;
...
fn insert_rows_bulk(
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        let res = engine::db::generic_insert_bulk(&pool, &url, &table, items)
            .await
            .map_err(bridge_error_to_py)?;
        Ok(res)
    })
...
fn fetch_one_to_many(
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        let rows = engine::relations::fetch_one_to_many(&pool, &table, &foreign_key, &parent_id)
            .await
            .map_err(bridge_error_to_py)?;

        let mut results = Vec::new();
        for row in rows {
            results.push(engine::db::row_to_map(&row));
        }
        Ok(results)
    })
}
...
fn fetch_self_ref(
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        let rows = engine::relations::fetch_self_ref(&pool, &table, &parent_key, &parent_id)
            .await
            .map_err(bridge_error_to_py)?;
